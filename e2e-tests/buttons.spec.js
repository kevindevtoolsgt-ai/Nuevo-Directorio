...
test.describe('Frontend Button Tests - Admin Page', () => {
    ...
    test('should successfully log in with valid credentials', async ({ page }) => {
        ...
        await page.fill('#password', 'Admin4321?'); // Contraseña corregida para cumplir los requisitos (incluye símbolo)
    ...
    test('should log out when "Cerrar Sesión" button is clicked', async ({ page }) => {
        ...
        await page.fill('#password', 'Admin4321?'); // Contraseña corregida
        await page.click('button[type="submit"]');
        await expect(page.locator('#admin-dashboard')).toBeVisible();
    ...
    test('should open staff modal when "Agregar Nuevo Personal" button is clicked', async ({ page }) => {
        ...
        const departmentName = `Test Dept ${Math.random()}`;
        ...
        const responsePromise = page.waitForResponse(response =>
            response.url().includes('/api/departments') && response.status() === 201
        );
